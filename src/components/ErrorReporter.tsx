...
export default function ErrorReporter({ error }: ReporterProps) {
  ...
  useEffect(() => {
    ...
    const send = (payload: unknown) => window.parent.postMessage(payload as any, "*");
        ...
        <div className="max-w-md w-full text-center space-y-6">
          <div className="space-y-2">
            ...
            <p className="text-muted-foreground">
              An unexpected error occurred. Please try again fixing with DAnh
            </p>
          </div>
            ...
            <details className="mt-4 text-left">
              ...
              <pre className="mt-2 text-xs bg-muted p-2 rounded overflow-auto whitespace-pre-wrap">
                ...
                {"digest" in error && (error as any).digest && (
                  <div className="mt-2 text-muted-foreground">Digest: {(error as any).digest}</div>
                )}
              </pre>
            </details>
          )}
